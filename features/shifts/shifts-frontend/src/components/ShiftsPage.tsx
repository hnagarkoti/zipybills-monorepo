    ...
    <ScrollView className="flex-1 p-4">
      ...
      {showForm && (
        <View className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 mb-4">
          <Text className="text-lg font-semibold mb-3 dark:text-gray-100">{editingId ? 'Edit Shift' : 'New Shift'}</Text>
          {!editingId && (
            <View className="mb-3">
              <Text className="text-xs text-gray-500 dark:text-gray-400 mb-1">Quick presets</Text>
              <View className="flex-row flex-wrap gap-2">
                {TIME_PRESETS.map((p) => (
                  <Pressable key={p.label} onPress={() => setForm({ ...form, shift_name: p.label.split(' (')[0] ?? p.label, start_time: p.start, end_time: p.end })} className="bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-lg">
                    <Text className="text-xs text-gray-600 dark:text-gray-400">{p.label}</Text>
                  </Pressable>
                ))}
              </View>
            </View>
          ...
          <View className="mb-3">
            <Text className="text-xs text-gray-500 dark:text-gray-400 mb-1">Shift Name *</Text>
            <TextInput className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2.5 text-sm dark:bg-gray-800 dark:text-gray-100" value={form.shift_name} onChangeText={(t) => setForm({ ...form, shift_name: t })} placeholder="e.g., Morning" />
          </View>
          <View className="flex-row gap-3 mb-3">
            <View className="flex-1">
              <Text className="text-xs text-gray-500 dark:text-gray-400 mb-1">Start Time * (HH:MM)</Text>
              <TextInput className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2.5 text-sm dark:bg-gray-800 dark:text-gray-100" value={form.start_time} onChangeText={(t) => setForm({ ...form, start_time: t })} placeholder="06:00" />
            </View>
            <View className="flex-1">
              <Text className="text-xs text-gray-500 dark:text-gray-400 mb-1">End Time * (HH:MM)</Text>
              <TextInput className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2.5 text-sm dark:bg-gray-800 dark:text-gray-100" value={form.end_time} onChangeText={(t) => setForm({ ...form, end_time: t })} placeholder="14:00" />
            </View>
          </View>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={<Clock size={40} color={sc.iconMuted} />}
          ...
          {shifts.map((s) => (
            <View key={s.shift_id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4 mb-2">
              <View className="flex-row items-center justify-between mb-1">
                ...
                <View className="flex-row gap-2">
                  <Pressable onPress={() => startEdit(s)} className="bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-lg">
