  ...
  return (
    <ScrollView className="flex-1 p-4">
      ...
      <View className="flex-row gap-2 mb-4 items-center">
        <View className="flex-1"><Text className="text-xs text-gray-500 dark:text-gray-400 mb-1">From</Text><TextInput className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 dark:text-gray-100" value={dateRange.start} onChangeText={(t) => setDateRange({ ...dateRange, start: t })} placeholder="YYYY-MM-DD" /></View>
        <View className="flex-1"><Text className="text-xs text-gray-500 dark:text-gray-400 mb-1">To</Text><TextInput className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 dark:text-gray-100" value={dateRange.end} onChangeText={(t) => setDateRange({ ...dateRange, end: t })} placeholder="YYYY-MM-DD" /></View>
      ...
      {loading ? (<Text className="text-center text-gray-400 py-8">Generating report...</Text>) : data.length === 0 ? (
        <EmptyState icon={<BarChart3 size={40} color={sc.iconMuted} />} title="No data to display" description="Set a date range and click Generate" />
      ) : (
        <View>
          {tab === 'production' && data.map((row, idx) => (
            <View key={idx} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-3 mb-2">
              ...
              <View className="flex-row gap-3"><Text className="text-sm text-gray-700 dark:text-gray-300">Target: <Text className="font-medium">{row.target_quantity}</Text></Text><Text className="text-sm text-green-600 dark:text-green-400">OK: <Text className="font-medium">{row.total_ok || 0}</Text></Text><Text className="text-sm text-red-600 dark:text-red-400">Rej: <Text className="font-medium">{row.total_rejected || 0}</Text></Text></View>
            </View>
          ))}
          {tab === 'machine' && data.map((row, idx) => {
            ...
            return (<View key={idx} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-3 mb-2">
              <View className="flex-row items-center justify-between mb-2"><Text className="font-semibold text-gray-900 dark:text-gray-100">{row.machine_name}</Text><Text className={`text-sm font-bold ${eff >= 90 ? 'text-green-600' : eff >= 70 ? 'text-yellow-600' : 'text-red-600'}`}>{eff.toFixed(1)}% eff</Text></View>
