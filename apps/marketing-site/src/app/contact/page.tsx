...
// Get it from: WhatsApp group > ⋮ > Invite via link > Copy link
const WHATSAPP_GROUP_LINK = 'https://chat.whatsapp.com/REPLACE_WITH_YOUR_GROUP_LINK';

// ← Personal number for the direct WhatsApp card (fallback)
...
export default function ContactPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    // Open the WhatsApp group — user can paste/send their message there
    // (Group links don't support pre-filled text, so we show the summary on-screen)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid sm:grid-cols-4 gap-6">
            ...
            >
              ...
              <div>
                <div className="text-sm text-emerald-700 font-medium">WhatsApp Group</div>
                <div className="font-semibold text-gray-900 text-sm">Join &amp; chat with us</div>
              </div>
            </a>
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-8">
                {submitted ? (
                  <div className="text-center py-16">
                    ...
                    <h3 className="text-2xl font-bold text-gray-900 mb-2">Join Our WhatsApp Group!</h3>
                    <p className="text-gray-500 max-w-xs mx-auto">
                      WhatsApp group is opening. Join and send your message — we reply directly in the group.
                    </p>
                    {/* Show the summary so user can copy-paste into the group */}
                    <div className="mt-5 text-left bg-gray-50 border border-gray-200 rounded-xl px-5 py-4 max-w-sm mx-auto text-sm text-gray-700 space-y-1 font-mono">
                      <div><span className="text-gray-400">Name:</span> {form.name}</div>
                      {form.company && <div><span className="text-gray-400">Company:</span> {form.company}</div>}
                      {form.phone && <div><span className="text-gray-400">Phone:</span> {form.phone}</div>}
                      {form.subject && <div><span className="text-gray-400">Subject:</span> {form.subject}</div>}
                      <div className="pt-1 border-t border-gray-200"><span className="text-gray-400">Message:</span> {form.message}</div>
                    </div>
                    <p className="text-xs text-gray-400 mt-3">Copy the above and paste it in the WhatsApp group.</p>
                    <a
                      href={WHATSAPP_GROUP_LINK}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="mt-4 inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-500 text-white font-semibold hover:bg-emerald-600 transition-colors"
                    >
